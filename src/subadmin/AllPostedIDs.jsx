...
export default function AllPostedIDs({ search = '' }) {
  ...
  const fetchPostedIDs = async () => {
    try {
      const res = await fetch('https://swarg-store-backend.onrender.com/api/ids/my', {
        headers: {
  ...
  const handleToggleStatus = async (id, currentStatus) => {
    try {
      ...
      setPostedIDs(ids => ids.map(row => row._id === id ? { ...row, status: newStatus } : row));
    } catch (err) {
  ...
  const handleDelete = async (id) => {
    try {
      ...
      setPostedIDs(ids => ids.filter(row => row._id !== id));
    } catch (err) {
      setError(err.message);
    }
  };

  const filtered = postedIDs.filter(
    (id) =>
  ...
  if (loading) return <div className="text-center py-12 text-cyan-400 font-bold animate-pulse">Loading...</div>;
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
        {filtered.length === 0 && !loading && (
          <div className="col-span-full text-center py-12">
            ...
            <p className="mt-2 text-gray-500">You haven't posted any IDs yet.</p>
          </div>
        )}
        {filtered.map((row) => {
